...
const API_URL = "https://script.google.com/macros/s/AKfycbzSEr8fLcS3XKP-Zv3XWOUwoffoYE5_ix6CNLnDK7cGzNze0a9W2BV1HzbZB-Rkpy4/exec";
...
const ai   = a => ({BLOCKED:"🔒",OUT:"🚗",RETURNED:"✅",UNBLOCKED:"🔓"}[a]||"•");
...
export default function TestDriveApp() {
  ...
  const [form, setForm]           = useState({ consultant:"", customer:"", expectedReturn:"" });
  ...
  const handleBlock = async () => {
    ...
    const ok = await doPost({ action:"blockCar", carId:modal.carId, carName:car?.name, consultant:form.consultant, customer:form.customer });
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"#0a0e1a", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", color:"#8892a4", gap:16 }}>
      ...
      <div style={{ fontSize:13 }}>Connecting to Google Sheets…</div>
    </div>
  );
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.hdr}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          {syncing && <div style={{ fontSize:10, color:"#f5a623" }}>Syncing…</div>}
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:12, fontWeight:600, color:"#e8c878" }}>{user.name}</div>
            <div style={{ fontSize:10, color:"#8892a4", textTransform:"uppercase", letterSpacing:"0.5px" }}>{user.role}</div>
          </div>
      ...
      {tab==="cars" && (
        <div style={S.page}>
          ...
          {/* Refresh hint */}
          <div style={{ fontSize:10, color:"#4a5568", textAlign:"center", marginBottom:10 }}>Auto-refreshes every 15s · <span style={{ cursor:"pointer", color:"#6b7a8d", textDecoration:"underline" }} onClick={fetchData}>Refresh now</span></div>
          ...
          {cars.map(car => {
            ...
            const isMyBlock = booking.consultant===user.name;
            const canRelease = user.role==="admin" || isMyBlock;
            return (
              <div key={car.id} style={{ ...S.card, border:`1px solid ${stColor}33`, boxShadow:`0 0 18px ${stColor}0d` }}>
                ...
                {booking.status!==STATUS.FREE && (
                  <div style={{ background:"rgba(0,0,0,0.2)", borderRadius:10, padding:"10px 12px", marginBottom:10, fontSize:12 }}>
                    ...
                    <div style={{ display:"flex", gap:14, flexWrap:"wrap", marginTop:5 }}>
                      {booking.blockedAt  && <div><span style={{ color:"#8892a4" }}>Blocked: </span><span style={{ color:"#f5a623" }}>{booking.blockedAt}</span></div>}
                      {booking.outSince   && <div><span style={{ color:"#8892a4" }}>Out since: </span><span style={{ color:"#e74c3c" }}>{booking.outSince}</span></div>}
                      {booking.expectedReturn && <div><span style={{ color:"#8892a4" }}>Return by: </span><span style={{ color:"#00c896" }}>{booking.expectedReturn}</span></div>}
                    </div>
                    {/* Lock notice */}
                    {booking.status===STATUS.BLOCKED && !canRelease && (
                      <div style={{ marginTop:8, fontSize:11, color:"#f5a623", background:"rgba(245,166,35,0.08)", borderRadius:6, padding:"5px 8px", border:"1px solid rgba(245,166,35,0.2)" }}>
                        🔐 Blocked by {booking.consultant} — only they or Admin can release
                      </div>
                    )}
                  </div>
                )}
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {booking.status===STATUS.FREE && (<>
                    <ABtn color="#f5a623" onClick={()=>{ setForm({consultant:user.name,customer:"",expectedReturn:""}); setModal({carId:car.id,action:"block"}); }}>🔒 Block</ABtn>
                    <ABtn color="#e74c3c" onClick={()=>{ setForm({consultant:user.name,customer:"",expectedReturn:""}); setModal({carId:car.id,action:"checkout"}); }}>🚗 Take Out</ABtn>
                  </>)}
                  {booking.status===STATUS.BLOCKED && (<>
                    <ABtn color="#e74c3c" onClick={()=>{ setForm({consultant:booking.consultant,customer:booking.customer,expectedReturn:""}); setModal({carId:car.id,action:"checkout_from_block"}); }}>🚗 Take Out Now</ABtn>
      ...
      {tab==="settings" && user.role==="admin" && (
        <div style={S.page}>
          ...
          <div style={S.card}>
            ...
            {cars.map(car=>{
              const booking = getBooking(car.id);
              const inUse   = booking.status!==STATUS.FREE;
              const stColor = sc(booking.status);
              return (
          ...
          <div style={{ fontSize:11, color:"#4a5568", textAlign:"center", paddingBottom:20 }}>Settings are visible to Admin only</div>
        </div>
      )}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:200, backdropFilter:"blur(4px)" }} onClick={()=>setModal(null)}>
          <div style={{ background:"#0f1624", borderRadius:"20px 20px 0 0", padding:"22px 18px 32px", width:"100%", maxWidth:480, border:"1px solid rgba(255,255,255,0.1)", boxShadow:"0 -20px 60px rgba(0,0,0,0.5)" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>
              {modal.action==="block"?"🔒 Block Car":modal.action==="checkout"?"🚗 Take Car Out":"🚗 Take Out (Pre-blocked)"}
            </div>
            <div style={{ fontSize:12, color:"#8892a4", marginBottom:18 }}>{getCar(modal.carId)?.name}</div>
